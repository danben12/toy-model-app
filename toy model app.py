...
with st.expander("About this App", expanded=False):
    st.markdown("""
    This application simulates and visualizes the growth of bacterial populations under varying antibiotic concentrations and droplet volumes using different mathematical models.
...
# <<< added dropdown menu for model selection
model_names = {
    "Effective Concentration": "effective_concentration_model",
    "Linear Lysis Rate": "linear_Lysis_rate_model",
    "Combined Model": "combined_model"
}
selected_model_name = st.sidebar.selectbox("Select Model", list(model_names.keys()))
# <<< end added dropdown
...
with st.sidebar.expander("Antibiotic Parameters", expanded=False):
    ...
    a = st.number_input("Growth-dependent Lysis (a)", 0.0, 10.0, 3.0, 0.1, format="%.2f", on_change=input_changed)
    b = st.number_input("Baseline Lysis (b)", 0.0, 10.0, 0.1, 0.05, format="%.2f", on_change=input_changed)
...
if __name__ == "__main__":
    ...
    model_func = model_map[selected_model_name]
    results = run_model_for_volumes(
        model_func, selected_model_name, volume, density, A_free0, t,
    ...
    p = figure(
        title=f'B_live over time - {selected_model_name}',
        x_axis_label='Time (h)',
        y_axis_label='B_live',
        y_axis_type='log',
        width=1400,
    ...
    hover = HoverTool(
        tooltips=[
            ("Time (h)", "@time{0.00}"),
            ("B_live", "@B_live{0.0000}"),
            ("Volume", "@volume{0,0}"),
            ("A₀", "@A0{0.0}")
        ],
        mode='vline'  # shows tooltip for nearest x-value
    )
    p.add_tools(hover)
